...
The .NET side uses `Tmds.DBus` and talks to `org.freedesktop.NetworkManager` plus the global `fi.w1.wpa_supplicant1.WFDIEs` property on the system bus. It does not invoke `nmcli`, `wpa_cli`, `iw`, `systemctl`, MiracleCast, LibVLC, `sudo`, `su`, `pkexec`, or a shell. The desktop user must be permitted by the distribution's D-Bus policy to access `fi.w1.wpa_supplicant1` (Debian-family systems normally grant this to the `netdev` group).
